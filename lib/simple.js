...
nativeFlowMessage: proto.Message.InteractiveMessage.NativeFlowMessage.create({buttons}),contextInfo:{ isForwarded: true, forwardingScore: 1000, forwardedNewsletterMessageInfo: { newsletterJid: "120363285522801458@newsletter", serverMessageId: 100, newsletterName: "Powered by: Lana Dev"}, mentionedJid: [...text.matchAll(/@([0-9]{5,16}|0)/g),...title.matchAll(/@([0-9]{5,16}|0)/g)].map((v) => v[1] + "@s.whatsapp.net") ||''}})}}}), {quoted, userJid: id})
...
//Serialize By Rulzz, Titenono lek ko Apus WM e lee😹🖕🏿

conn.sendListButton1= async (jid, text, list, footer, quoted, options = {}) => {
  ...
  pack: "lan", // The pack name
  author: "By LanaBotz", // The author name
...
return await conn.sendMessage(jid, { [`${pase}`]: data, mimetype: mime, ...options }, { quoted })
  ...
  */
  ...
  +"item2.X-ABLabel:Email\nitem3.URL:https://instagram.com/whyzzxy\n"
...
let image = null;
if (options.img) {
image = await prepareWAMessageMedia({ image: {url:options.img} }, { upload: conn.waUploadToServer });
} else {
...
newsletterJid: "120363285522801458@newsletter",
serverMessageId: 100,
newsletterName: "Powered By LANA BOTZ"
},
mentionedJid: [
